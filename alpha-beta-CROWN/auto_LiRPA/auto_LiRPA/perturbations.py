...
class PerturbationLpNorm(Perturbation):
    ...
    def __init__(self, eps=0, norm=np.inf, x_L=None, x_U=None, eps_min=0):
        ...
        self.x_U = x_U
        self.sparse = False
        self.bitmask = torch.ones((28, 28)).flatten().cuda()
    ...
    def concretize_matrix(self, x, A, sign):
        ...
        if self.norm == np.inf:
            ...
            x_ub = x_U.reshape(x_U.shape[0], -1, 1) # shape (1, 784, 1)
            x_lb = x_L.reshape(x_L.shape[0], -1, 1)
            # Find the uppwer and lower bound similarly to IBP.
            center = (x_ub + x_lb) / 2.0
            diff = (x_ub - x_lb) / 2.0
            if not isinstance(A, eyeC):
                bound = A.matmul(center) + sign * (A*self.bitmask.unsqueeze(0).unsqueeze(0)).abs().matmul(diff)
            else:
                # A is an identity matrix. No need to do this matmul.
                bound = center + sign * diff * self.bitmask.unsqueeze(0).unsqueeze(-1)
        else:
            x = x.reshape(x.shape[0], -1, 1)
            if not isinstance(A, eyeC):
                # Find the upper and lower bounds via dual norm.
                if self.bitmask is not None:
                    deviation = (A * self.bitmask.unsqueeze(0).unsqueeze(0)).norm(self.dual_norm, -1) * self.eps
                else:
    ...
    def concretize_patches(self, x, A, sign):
        if self.norm == np.inf:
            ...
            masked_diff = diff * self.bitmask.unsqueeze(0).unsqueeze(0).reshape(1, 1, diff.shape[-1], diff.shape[-1])

            if not A.identity == 1:
                bound = A.matmul(center)
                bound_diff = A.matmul(masked_diff, patch_abs=True)
